...
Place in ms/ folder and create ms/.env with HF_TOKEN and TESSERACT_CMD values.
...
def hf_inference(prompt: str,
    ...
    if not HF_TOKEN:
        return {"text": "HF_TOKEN missing in .env", "latency": 0.0, "tokens_total": None}
    ...
    try:
        ...
        return {"text": "Hugging Face call failed: could not parse JSON", "latency": latency, "tokens_total": None}
...
async def image_qa(file: UploadFile = File(...), question: str = Form(...)):
    ...
    out = call_openai_chat(prompt, max_tokens=350)
    log_metrics("image_qa", out.get("latency", 0), f"{DEFAULT_HF_MODEL} + pytesseract", out.get("tokens_total"))
